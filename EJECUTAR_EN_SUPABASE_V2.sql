-- ============================================
-- SCRIPT DE REPARACIÓN - VERSIÓN 2
-- Ejecuta esto en el SQL Editor de Supabase
-- ============================================

-- Esta versión actualizada de la función NO lanza error si el trimestre ya existe
-- En su lugar, lo devuelve directamente

DROP FUNCTION IF EXISTS create_trimestre_with_checklist(UUID, INT, INT);
...
BEGIN
  ...
  -- PRIMERO: Verificar si ya existe el trimestre
  SELECT id INTO v_existing_trimestre_id
  FROM trimestres
  ...
  IF FOUND THEN
    SELECT json_build_object(
      ...
      'expected_documents', (
        ...
        WHERE ed.trimestre_id = t.id
      )
    )
    INTO v_trimestre
    FROM trimestres t
    WHERE t.id = v_existing_trimestre_id;
...
COMMENT ON FUNCTION create_trimestre_with_checklist IS 'Crea o devuelve un trimestre existente con sus checklist items y documentos esperados';

-- ============================================
-- VERIFICACIÓN
-- ============================================

-- Ejecuta esta query para verificar que la función se actualizó correctamente:
